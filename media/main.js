...
async function displayMessage(msg, ch, limited=0) {
  ...
  ${msg.user.hide?`<span class="time">${formatHour(msg.timestamp)}</span>`:`<div class="avatar"><img src="${msg.user.pfp?pfpById(msg.user.pfp):userToDefaultPfp(msg.user)}" width="42" height="42" aria-hidden="true" draggable="false" loading="lazy" onerror="this.src='${userToDefaultPfp(msg.user)}'"></div>`}
  <div class="inner">
    <div class="actions">
      ${limited===0?`
...
`<button onclick="navigator.clipboard.writeText('${btn.getAttribute('username')}')" tlang="member.copyusername">Copy username</button>`+
(window.username!==btn.getAttribute('username')&&(hasPerm(ch.permission,Permissions.MANAGE_PERMISSION)||hasPerm(ch.permission,Permissions.MANAGE_MEMBERS))?'<hr style="width:90%">':'')+
...
(window.username===btn.getAttribute('username')?'':
...
<button onclick="window.banmember('${btn.getAttribute('username')}')" class="danger" tlang="member.ban">Ban</button>`:'')),
      interactive: true,
      trigger: 'click',
    ...
    btn.oncontextmenu = (evt)=>{
      evt.preventDefault();
      tip[tip.state.isVisible?'hide':'show']();
    };
  });
}
...
window.bansPanel = ()=>{
  ...
  <img src="${ban.pfp?pfpById(ban.pfp):userToDefaultPfp(ban)}" width="30" height="30" aria-hidden="true" draggable="false" loading="lazy" onerror="this.src='${userToDefaultPfp(ban)}'">
  <span>
  ...
  <button onclick="window.unbanmember('${sanitizeMinimChars(ban.username)}')">x</button>
        ...
        .join('')||'<span tlang="member.nobans">There are no banned members</span>';
    });
};
window.invitePanel = ()=>{
...
window.useredit = ()=>{
  ...
  <img src="${usr.pfp?pfpById(usr.pfp):userToDefaultPfp(usr)}" width="30" height="30" aria-hidden="true" draggable="false" loading="lazy" onerror="this.src='${userToDefaultPfp(usr)}'">
  <span>
    <span>${sanitizeHTML(usr.display??sanitizeMinimChars(usr.username))}</span>
...
<button onclick="logout()" tlang="user.logout" style="color:var(--red)">Log out</button>`,
  ...
  document.getElementById('user').oncontextmenu = (evt)=>{
    evt.preventDefault();
    usertip[usertip.state.isVisible?'hide':'show']();
  };
